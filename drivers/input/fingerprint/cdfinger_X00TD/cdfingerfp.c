...
static int cdfinger_init_irq(struct cdfingerfp_data *pdata)
{
	...
	error =request_irq(gpio_to_irq(pdata->irq_num),cdfinger_eint_handler,IRQF_TRIGGER_RISING | IRQF_PERF_CRITICAL,"cdfinger_eint", NULL);
	if (error < 0)
	{
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	CDFINGER_DBG("cdfinger_probe in \n");
	...
	CDFINGER_DBG("cdfinger_probe ok \n");
